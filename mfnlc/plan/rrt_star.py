...
class RRTStar(RRT):
    def search(self,
        ...
        for i in range(max_iteration):
            ...
            if not collision:
                ...
                new_parent, new_cost, new_configurations = self.choose_parent(sampled_vertex, near_vertices)
                if new_parent is not None:
                    self.tree.insert_vertex(new_parent, sampled_vertex)
                    if self.with_dubins_curve:
                        sampled_vertex.kinodynamically_feasible_path = new_configurations
    ...
    def get_near_vertices(self,
        ...
        dist = np.linalg.norm((self.tree.all_vertices_state - sampled_vertex.state)[:, :2], axis=-1)
        indx = dist < radius
    ...
    def rewire(self,
        ...
        for child in near_vertices:
            collision, new_cost, new_configurations = self._steer(sampled_vertex, child)
            ...
            if not collision and cost_diff < 0:
                ...
                if self.with_dubins_curve:
                    child.kinodynamically_feasible_path = new_configurations
