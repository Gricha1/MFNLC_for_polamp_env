...
class SafetyRis(SAC):
    ...
    def train_highlevel_policy(self, state, goal, subgoal, debug_info={}):
        ...
        with th.no_grad():
            ...
            policy_v = th.cat([policy_v_1, policy_v_2], -1).clamp(min=self.clip_v_function - self.env.envs[0].env.arrive_reward, max=0.0).abs().min(-1)[0]
            ...
            v = th.cat([v_1, v_2], -1).clamp(min=self.clip_v_function - self.env.envs[0].env.arrive_reward, max=0.0).abs().min(-1)[0]
